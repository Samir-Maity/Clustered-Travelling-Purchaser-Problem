...
f = open('p_cost_file_swiss42.txt','r')
Lines = f.read()
...
#        print("Routes lengths:", routes_length, "\n")
    ...
    for i in range(len(best_index)):
        purch_index_ucost.append(pur_cst_of_p[best_index[i]])
    purch_cost = 0
    for i in range(len(best_index)):
        purch_cost = purch_cost + (purch_index[i]*purch_index_ucost[i]). ### Purchasing Cost  Calculations
    #purch_cost = b_path - p_length
    print("Purchasing Cost = ",purch_cost)
    print("TOTAL Cost = ",purch_cost+p_length)
